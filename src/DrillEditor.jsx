...
export default function DrillEditor({ onBack, session, isAdmin, settings }) {
  ...
  function handlePrev() { if (lépésIndex > 0) { setLépésIndex(lépésIndex - 1); setGame(new Chess(history[lépésIndex - 1].fen)); setOptionSquares({}); setMoveFrom(''); } }
  function handleNext() { if (lépésIndex < history.length - 1) { setLépésIndex(lépésIndex + 1); setGame(new Chess(history[lépésIndex + 1].fen)); setOptionSquares({}); setMoveFrom(''); } }
  ...
  function onDrop(source, target) {
    if (source === target) { setMoveFrom(source); if (settings?.showLegalMoves !== false) getMoveOptions(source); return false; }
  ...
  return (
    <div className="center-container">
      ...
      <div className="card">
        ...
        <input className="input-field" placeholder="Mappa neve" value={editKategoria} onChange={(e) => setEditKategoria(e.target.value)} style={{ marginBottom: '20px' }} />
      ...
      <DrillList drillLista={drillLista} onEdit={startEditing} onDelete={async (nev) => { if(window.confirm('Biztosan törlöd ezt a variációt?')) { await supabase.from('variaciok').delete().eq('nev', nev); fetchList(); } }} isAdmin={isAdmin} />
    </div>
  );
}